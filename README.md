...
This project presents a fully automated, CUDA-12.8–compatible environment for running ComfyUI on Kaggle Notebooks, enabling high-performance diffusion model inference, video generation, and multimodal workflows in a cloud-limited environment. The script constructs a complete, reproducible runtime by installing system dependencies, enforcing Python package constraints for compatibility, and provisioning a customized PyTorch 2.9.0 (cu128) stack with all required NVIDIA CUDA libraries. It further integrates GPU-accelerated toolchains—including xFormers, ONNXRuntime GPU, MediaPipe, and Segment Anything v2—to support advanced workflows such as frame interpolation, I2V, video pipelines, and model distillation.

A fully modular plugin manager installs and configures all necessary ComfyUI extensions, including Kijai’s WanVideo workflow requirements, control networks, video helper tools, math utilities, and auxiliary node suites. The script also links or copies pre-downloaded model files (GGUF, text encoders, VAEs, LoRAs, and vision modules) directly from Kaggle Datasets while maintaining a clean directory structure compatible with ComfyUI Manager. To support inference at scale, it launches ComfyUI in background mode with automated logging, HTTP health checks, and adaptive VRAM configuration.

By encapsulating system setup, dependency resolution, plugin installation, and server orchestration into a single executable workflow, this project enables reliable, high-performance generative AI execution on constrained Kaggle infrastructure. The resulting environment provides a turnkey solution for researchers, developers, and artists to run modern diffusion and video models—including Wan2.2 Animate without local GPU resources.

This repository contains the notebooks and workflows specifically configured to run inference from Wan 2.2 Animate with ComfyUI on Kaggle T4 GPUs smoothly. There are 3 different types of notebooks and 3 different variants of Kijai's modified workflows.
